...
| <A NAME="Video.Stream.HEVC"></A>`Video.Stream.HEVC`                                 | Stream native H.265 (HEVC) video without transcoding to HomeKit clients that support it (iOS 27 / tvOS 27 and later). Requires a camera that is encoding in H.265 and a HEVC-capable build of Homebridge (see the HEVC & 4K Streaming documentation). Has no effect on H.264 or AV1 cameras. **(default: enabled)**.
